...
class MetricLoader:
    ...
    @timer
    def get_unique_pcp_labels(self, start: datetime, end: datetime, metrics: List[str]) -> List[str]:
        """Gets the unique machine ids based on the metrics"""
        pod_ids = self.get_unique_label(start, end, metrics, label_name="machineid")
        if not pod_ids:
            logger.warning(f'Empty unique pcp ids on given metrics!')
            logger.debug(f'metrics: {";".join(metrics)}')
        else:
            logger.info(f'Got {len(pod_ids)} unique pcp ids on given metrics')
            logger.debug(f'pcp ids: {";".join(pod_ids)}')
            logger.debug(f'metrics: {";".join(metrics)}')
